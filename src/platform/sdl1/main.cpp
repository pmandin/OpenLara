...
void osMutexFree(void *obj) {
    SDL_DestroyMutex( (SDL_mutex *) obj);
}

void osMutexLock(void *obj) {
    SDL_mutexP( (SDL_mutex *) obj);
}

void osMutexUnlock(void *obj) {
    SDL_mutexV( (SDL_mutex *) obj);
}
#endif
...
int main() {
	Uint32	startTick, curTick, delayMs;

    SDL_Init(SDL_INIT_VIDEO | SDL_INIT_NOPARACHUTE);
    SDL_ShowCursor(0);
    screen = SDL_SetVideoMode(SCREEN_WIDTH, SCREEN_HEIGHT, 16, /*SDL_DOUBLEBUF |*/ SDL_SWSURFACE /*| SDL_NOFRAME*/);
...
//    sndInit();
    ...
    while (!isQuit) {
		...
		if (Game::update()) {
			...
			delayMs = curTick - startTick;
			if (delayMs < 17) {
				SDL_Delay(17-delayMs);
			}
			startTick = curTick;
		}
	}
